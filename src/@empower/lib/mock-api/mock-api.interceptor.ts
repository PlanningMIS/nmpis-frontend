...
import { EMPOWER_MOCK_API_DEFAULT_DELAY } from '@empower/lib/mock-api/mock-api.constants';
import { EmpowerMockApiService } from '@empower/lib/mock-api/mock-api.service';
...
export const mockApiInterceptor = (
    ...
    const defaultDelay = inject(EMPOWER_MOCK_API_DEFAULT_DELAY);
    const empowerMockApiService = inject(EmpowerMockApiService);

    // Try to get the request handler
    const { handler, urlParams } = empowerMockApiService.findHandler(
