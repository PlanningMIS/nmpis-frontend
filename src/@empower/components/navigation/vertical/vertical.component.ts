...
import {
    ...
    EmpowerVerticalNavigationAppearance,
    EmpowerVerticalNavigationMode,
    EmpowerVerticalNavigationPosition,
} from '@empower/components/navigation/navigation.types';
import { EmpowerVerticalNavigationAsideItemComponent } from '@empower/components/navigation/vertical/components/aside/aside.component';
...
import { EmpowerScrollbarDirective } from '@empower/directives/scrollbar/scrollbar.directive';
...
@Component({
    selector: 'empower-vertical-navigation',
    templateUrl: './vertical.component.html',
    styleUrls: ['./vertical.component.scss'],
    animations: empowerAnimations,
    encapsulation: ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush,
    exportAs: 'empowerVerticalNavigation',
    standalone: true,
    imports: [
        EmpowerScrollbarDirective,
        EmpowerVerticalNavigationAsideItemComponent,
...
export class EmpowerVerticalNavigationComponent
    implements OnChanges, OnInit, AfterViewInit, OnDestroy
{
    ...
    @Input() appearance: EmpowerVerticalNavigationAppearance = 'default';
    @Input() autoCollapse: boolean = true;
    @Input() inner: boolean = false;
    @Input() mode: EmpowerVerticalNavigationMode = 'side';
    @Input() name: string = this._empowerUtilsService.randomId();
    @Input() navigation: EmpowerNavigationItem[];
    @Input() opened: boolean = true;
    @Input() position: EmpowerVerticalNavigationPosition = 'left';
    @Input() transparentOverlay: boolean = false;
    @Output()
    readonly appearanceChanged: EventEmitter<EmpowerVerticalNavigationAppearance> =
        new EventEmitter<EmpowerVerticalNavigationAppearance>();
    @Output() readonly modeChanged: EventEmitter<EmpowerVerticalNavigationMode> =
        new EventEmitter<EmpowerVerticalNavigationMode>();
    @Output() readonly openedChanged: EventEmitter<boolean> =
        new EventEmitter<boolean>();
    @Output()
    readonly positionChanged: EventEmitter<EmpowerVerticalNavigationPosition> =
        new EventEmitter<EmpowerVerticalNavigationPosition>();
    @ViewChild('navigationContent') private _navigationContentEl: ElementRef;

    activeAsideItemId: string | null = null;
    onCollapsableItemCollapsed: ReplaySubject<EmpowerNavigationItem> =
        new ReplaySubject<EmpowerNavigationItem>(1);
    onCollapsableItemExpanded: ReplaySubject<EmpowerNavigationItem> =
        new ReplaySubject<EmpowerNavigationItem>(1);
    ...
    private _empowerScrollbarDirectives!: QueryList<EmpowerScrollbarDirective>;
    private _empowerScrollbarDirectivesSubscription: Subscription;
    ...
    @HostBinding('class') get classList(): any {
        ...
        return {
            ...
            [`empower-vertical-navigation-appearance-${this.appearance}`]: true,
            'empower-vertical-navigation-hover': this._hovered,
            'empower-vertical-navigation-inner': this.inner,
            'empower-vertical-navigation-mode-over': this.mode === 'over',
            'empower-vertical-navigation-mode-side': this.mode === 'side',
            'empower-vertical-navigation-opened': this.opened,
            'empower-vertical-navigation-position-left': this.position === 'left',
    ...
    /**
     * Setter for empowerScrollbarDirectives
     */
    @ViewChildren(EmpowerScrollbarDirective)
    set empowerScrollbarDirectives(
        empowerScrollbarDirectives: QueryList<EmpowerScrollbarDirective>
    ) {
        // Store the directives
        this._empowerScrollbarDirectives = empowerScrollbarDirectives;

        // Return if there are no directives
        if (empowerScrollbarDirectives.length === 0) {
            return;
        }

        // Unsubscribe the previous subscriptions
        if (this._empowerScrollbarDirectivesSubscription) {
            this._empowerScrollbarDirectivesSubscription.unsubscribe();
        }

        // Update the scrollbars on collapsable items' collapse/expand
        this._empowerScrollbarDirectivesSubscription = merge(
            ...
            .subscribe(() => {
                ...
                empowerScrollbarDirectives.forEach((empowerScrollbarDirective) => {
                    empowerScrollbarDirective.update();
                });
            });
    }
    ...
    ngAfterViewInit(): void {
        ...
        setTimeout(() => {
            ...
            // Otherwise
                ...
                this._empowerScrollbarDirectives.forEach(
                    (empowerScrollbarDirective) => {
                        // Skip if not enabled
                        if (!empowerScrollbarDirective.isEnabled()) {
                            return;
                        }

                        // Scroll to the active element
                        empowerScrollbarDirective.scrollToElement(
                            '.empower-vertical-navigation-item-active',
                            -120,
                            true
                        );
     ...
     * @param item
     */
    openAside(item: EmpowerNavigationItem): void {
     ...
     * @param item
     */
    toggleAside(item: EmpowerNavigationItem): void {
        // Toggle
    ...
    private _showOverlay(): void {
        ...
        this._overlay.classList.add('empower-vertical-navigation-overlay');

        // Add a class depending on the transparentOverlay option
        if (this.transparentOverlay) {
            this._overlay.classList.add(
                'empower-vertical-navigation-overlay-transparent'
            );
        }
