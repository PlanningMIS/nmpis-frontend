...
import {
    ScrollbarGeometry,
    ScrollbarPosition,
} from '@empower/directives/scrollbar/scrollbar.types';
 ...
 */
@Directive({
    selector: '[empowerScrollbar]',
    exportAs: 'empowerScrollbar',
    standalone: true,
})
export class EmpowerScrollbarDirective implements OnChanges, OnInit, OnDestroy {
    /* eslint-disable @typescript-eslint/naming-convention */
    static ngAcceptInputType_empowerScrollbar: BooleanInput;
    /* eslint-enable @typescript-eslint/naming-convention */

    private _elementRef = inject(ElementRef);
    private _platform = inject(Platform);

    @Input() empowerScrollbar: boolean = true;
    @Input() empowerScrollbarOptions: PerfectScrollbar.Options;
    ...
    ngOnChanges(changes: SimpleChanges): void {
        // Enabled
        if ('empowerScrollbar' in changes) {
            // Interpret empty string as 'true'
            this.empowerScrollbar = coerceBooleanProperty(
                changes.empowerScrollbar.currentValue
            );

            // If enabled, init the directive
            if (this.empowerScrollbar) {
                this._init();
            }
        ...
        // Scrollbar options
        if ('empowerScrollbarOptions' in changes) {
            // Merge the options
            this._options = merge(
                {},
                this._options,
                changes.empowerScrollbarOptions.currentValue
            );
